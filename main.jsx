...
function SpeciesCard({ species, phylumNum, className, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:"1rem"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:IVORY,borderRadius:"16px",width:"100%",maxWidth:"560px",maxHeight:"88vh",overflowY:"auto",boxShadow:"0 32px 100px rgba(0,0,0,0.4)"}}>

        {/* Phylum-coloured header */}
        <div style={{background:colors.bg,borderRadius:"16px 16px 0 0",padding:"1.3rem 1.5rem 1rem",flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div>
              <div style={{fontSize:"0.6rem",color:colors.accent,letterSpacing:"0.15em",textTransform:"uppercase",fontFamily:"Georgia,serif",marginBottom:"0.3rem",opacity:0.8}}>
          ...
          <div style={{display:"flex",gap:"0.35rem",flexWrap:"wrap",marginTop:"0.7rem"}}>
            <span style={{background:"rgba(255,255,255,0.13)",color:colors.accent,padding:"0.12rem 0.55rem",borderRadius:"20px",fontSize:"0.63rem",fontFamily:"serif"}}>
              Phylum {phylumNum}
            </span>
            {entry?.rarity && (
              <span style={{background:RARITY_COLOR[entry.rarity]||MUTED,color:"#fff",padding:"0.12rem 0.55rem",borderRadius:"20px",fontSize:"0.63rem",fontFamily:"serif"}}>
                {entry.rarity}
              </span>
            )}
        ...
        <div style={{padding:"1.25rem 1.5rem 1.5rem"}}>
          ...
          {/* Sensitive banner */}
          {sensitive && (
            <div style={{background:"#FFF5F5",border:"1px solid #FECACA",borderLeft:`4px solid ${RED}`,borderRadius:"8px",padding:"0.7rem 1rem",marginBottom:"1rem"}}>
          ...
          {entry ? (
            <>
              <p style={{fontFamily:"Georgia,serif",fontSize:"1rem",color:INK,lineHeight:1.75,margin:"0 0 1rem",fontStyle:"italic"}}>
                {entry.summary}
              </p>
              <div style={{display:"flex",gap:"0.5rem",flexWrap:"wrap",marginBottom:"1.1rem"}}>
                {entry.era && (
                  <div style={{background:SOFT,border:`1px solid ${RULE}`,borderRadius:"8px",padding:"0.4rem 0.75rem"}}>
                    <div style={{fontSize:"0.58rem",color:MUTED,fontFamily:"serif",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:"0.15rem"}}>Era</div>
                    <div style={{fontFamily:"Georgia,serif",fontSize:"0.82rem",color:INK}}>{entry.era}</div>
                  </div>
                )}
                {entry.mood && (
                  <div style={{background:SOFT,border:`1px solid ${RULE}`,borderRadius:"8px",padding:"0.4rem 0.75rem",flex:1,minWidth:"140px"}}>
                    <div style={{fontSize:"0.58rem",color:MUTED,fontFamily:"serif",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:"0.15rem"}}>Defining Mood</div>
              ...
              {/* Expand button */}
              <button onClick={()=>setExpanded(ex=>!ex)} style={{width:"100%",padding:"0.6rem 1rem",background:expanded?colors.bg:SOFT,color:expanded?colors.accent:MUTED,border:`1px solid ${expanded?colors.accent:RULE}`,borderRadius:"8px",cursor:"pointer",fontFamily:"Georgia,serif",fontSize:"0.82rem",display:"flex",justifyContent:"space-between",alignItems:"center",transition:"all 0.2s",marginBottom:expanded?"1rem":0}}>
                <span>{expanded?"Collapse entry":"Read full field guide entry"}</span>
              ...
              {expanded && (
                <div style={{display:"flex",flexDirection:"column",gap:"0.85rem"}}>
                  ...
                  {entry.see_also?.length>0 && (
                    <div style={{background:SOFT,border:`1px solid ${RULE}`,borderRadius:"8px",padding:"0.9rem 1rem"}}>
                      <div style={{fontSize:"0.6rem",color:MUTED,fontFamily:"serif",letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:"0.6rem"}}>See Also</div>
                      <div style={{display:"flex",flexWrap:"wrap",gap:"0.35rem"}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0.6rem",paddingTop:"0.5rem",borderTop:`1px solid ${RULE}`}}>
            ...
            /* No entry yet — show existing note if available */
            <div style={{background:SOFT,borderRadius:"8px",border:`1px solid ${RULE}`,padding:"0.9rem 1rem"}}>
              {noteData?.note ? (
                ...
                <>
                  <div style={{fontSize:"0.6rem",color:MUTED,fontFamily:"serif",letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:"0.4rem"}}>Full Entry Forthcoming</div>
                  <p style={{fontFamily:"Georgia,serif",fontSize:"0.85rem",color:MUTED,margin:0,lineHeight:1.6,fontStyle:"italic"}}>
                    Era of origin, defining characteristics, key garments, and natural habitat — being added phylum by phylum.
                  </p>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0.6rem",paddingTop:"0.75rem",marginTop:"0.75rem",borderTop:`1px solid ${RULE}`}}>
                    {[["Kingdom","Fashion Aesthetic Culture"],["Phylum",`${phylumNum} · ${phylumName}`],["Class",className],["Species",name]].map(([label,value])=>(
                      <div key={label}>
                        <div style={{fontSize:"0.58rem",color:MUTED,fontFamily:"serif",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:"0.15rem"}}>{label}</div>
                        <div style={{fontSize:"0.78rem",color:INK,fontFamily:"Georgia,serif"}}>{value}</div>
  ...
  return (
    <div>
      <div style={{display:"flex",flexWrap:"wrap",gap:"0.35rem",marginBottom:"1.5rem"}}>
        <button onClick={()=>setActiveClass(null)} style={{padding:"0.3rem 0.8rem",borderRadius:"20px",border:"none",background:activeClass===null?colors.bg:SOFT,color:activeClass===null?colors.accent:MUTED,cursor:"pointer",fontSize:"0.7rem",fontFamily:"serif",transition:"all 0.15s"}}>All classes</button>
        {phylum.classes.map(c=>(
          <button key={c.name} onClick={()=>setActiveClass(activeClass===c.name?null:c.name)} style={{padding:"0.3rem 0.8rem",borderRadius:"20px",border:"none",background:activeClass===c.name?colors.bg:SOFT,color:activeClass===c.name?colors.accent:MUTED,cursor:"pointer",fontSize:"0.7rem",fontFamily:"serif",transition:"all 0.15s"}}>{c.name}</button>
        ))}
      </div>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:"0.4rem"}}>
              {cls.species.map(s=>{
                ...
                return (
                  ...
                  >
                    {isS&&"⚠ "}{sname}{isX&&" ◆"}
                    {(hasEntry||hasNote)&&<span style={{width:5,height:5,background:colors.accent,borderRadius:"50%",display:"inline-block",marginLeft:5,verticalAlign:"middle",opacity:0.7}}/>}
                  </button>
                );
              })}
            </div>
...
function SearchView({ query, onSpeciesClick }) {
    ...
    for (const p of ATLAS_DATA.phyla) {
      for (const c of p.classes) {
        for (const s of c.species) {
          ...
          const inNote=nd?.note?.toLowerCase().includes(q);
          const inCross=nd?.cross?.toLowerCase().includes(q);
          const inEntry=ent?.summary?.toLowerCase().includes(q)||ent?.context?.toLowerCase().includes(q);
          if (inName||inNote||inCross||inEntry) {
            matches.push({species:s,phylumNum:p.number,phylumName:p.name,phylumEmoji:p.emoji,className:c.name,matchType:inName?"name":inNote?"note":inCross?"crossover":"entry"});
  ...
  if (!query||query.length<2) return (
    <div style={{textAlign:"center",padding:"5rem 2rem",color:MUTED,fontFamily:"Georgia,serif"}}>
      ...
      <div style={{fontStyle:"italic",fontSize:"1rem"}}>Search all {ATLAS_DATA.total.toLocaleString()} species, field notes, and crossover tags…</div>
    </div>
  );
  ...
  return (
    <div>
      <div style={{marginBottom:"1.25rem",color:MUTED,fontFamily:"serif",fontSize:"0.85rem"}}>
        {results.length===100?"100+ results":`${results.length} result${results.length!==1?"s":""}`}{" "}for <strong style={{color:INK}}>"{query}"</strong>
...
function StatsView() {
  const stats = useMemo(()=>{
    let sensitive=0,crossover=0,withNotes=0,withEntries=0;
    ...
    return {sensitive,crossover,withNotes,withEntries,totalClasses:ATLAS_DATA.phyla.reduce((a,p)=>a+p.classes.length,0)};
  },[]);
...
export default function App() {
  ...
  function goHome() {
    setView("home"); setActivePhylum(null); setSearchQuery(""); setSelectedSpecies(null);
  }
          ...
          <div>
            <div style={{textAlign:"center",padding:"2.5rem 0 3rem",borderBottom:`1px solid ${ACCENT}`,marginBottom:"2rem"}}>
              ...
              <p style={{color:INK,fontFamily:"Georgia,serif",fontSize:"0.85rem",maxWidth:"500px",margin:"0 auto",lineHeight:1.8,opacity:0.75}}>
                Click any species to open its full field guide entry — era, visual description, key garments, habitat, and cultural context. Species with a dot ● have entries.
              </p>
            </div>
